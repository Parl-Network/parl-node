...
use parl_common::utils::spawn_task;
use parl_wallet::{
    wallet::Wallet,
...
pub struct Config {
    ...
    /// If disabled, the log file will be named parl-wallet.log instead of YYYY-MM-DD.parl-wallet.log
    #[clap(long)]
    ...
    /// Log filename
    /// 
    /// By default filename is parl-wallet.log.
    /// File will be stored in logs directory, this is only the filename, not the full path.
    /// Log file is rotated every day and has the format YYYY-MM-DD.parl-wallet.log.
    #[clap(long, default_value_t = String::from("parl-wallet.log"))]
    filename_log: String,
    /// Logs directory
    /// 
    ...
    if let Some(manager) = command_manager {
        ...
        if let Ok(wallet) = context.get::<Arc<Wallet>>() {
            ...
            let balance = format!(
                ...
                prompt.colorize_string(Color::Green, &format_parl(storage.get_plaintext_balance_for(&PARL_ASSET).await.unwrap_or(0))),
            );
            ...
            return Ok(
                format!(
                    "{} | {} | {} | {} | {} {}{} ",
                    prompt.colorize_str(Color::Blue, "PARL Wallet"),
                    addr_str,
                    topoheight_str,
                    balance,
                    status,
...
async fn transfer_all(manager: &CommandManager, mut args: ArgumentManager) -> Result<(), CommandError> {
    ...
    manager.message(format!("Sending {} of {} to {} (fees: {})", format_coin(amount, decimals), asset, address.to_string(), format_parl(estimated_fees)));
