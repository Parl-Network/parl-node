...
impl<'a, S: Storage> ChainState<'a, S> {
    ...
    pub async fn reward_miner(&mut self, miner: &'a PublicKey, reward: u64) -> Result<(), BlockchainError> {
        debug!("Rewarding miner {} with {} XTR at topoheight {}", miner.as_address(self.storage.is_mainnet()), format_parl(reward), self.topoheight);
        let miner_balance = self.internal_get_receiver_balance(miner, &PARL_ASSET).await?;
...
impl<'a, S: Storage> BlockchainVerificationState<'a, BlockchainError> for ChainState<'a, S> {
    ...
    async fn pre_verify_tx<'b>(
        ...
        if required_fees > tx.get_fee() {
            debug!("Invalid fees for tx {}: {} required, {} provided", tx.hash(), format_parl(required_fees), format_parl(tx.get_fee()));
