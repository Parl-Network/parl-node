...
use parl_common::{
    ...
    utils::{calculate_tx_fee, format_parl, spawn_task},
    varuint::VarUint
};
use crate::{
    config::{
...
impl<S: Storage> Blockchain<S> {
    ...
    async fn create_genesis_block(&self) -> Result<(), BlockchainError> {
        ...
        // register PARL asset
        debug!("Registering PARL asset: {} at topoheight 0", PARL_ASSET);
        storage.add_asset(&PARL_ASSET, AssetData::new(0, COIN_DECIMALS)).await?;
    ...
    // Get the current emitted supply of PARL at current topoheight
    ...
    pub async fn get_block_template_for_storage(&self, storage: &S, address: PublicKey) -> Result<BlockHeader, BlockchainError> {
        ...
        while let Some(TxSelectorEntry { size, hash, tx }) = tx_selector.next() {
            ...
            if let Err(e) = tx.verify(&mut chain_state).await {
                ...
                trace!("Selected {} (nonce: {}, fees: {}) for mining", hash, tx.get_nonce(), format_parl(tx.get_fee()));
                // TODO no clone
