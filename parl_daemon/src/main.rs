...
#[derive(Parser)]
#[clap(version = VERSION, about = "PARL: An innovate cryptocurrency with BlockDAG and Homomorphic Encryption enabling Smart Contracts")]
#[command(styles = parl_common::get_cli_styles())]
pub struct NodeConfig {
    ...
    /// If disabled, the log file will be named parl-daemon.log instead of YYYY-MM-DD.parl-daemon.log
    #[clap(long)]
    ...
    /// Log filename
    /// 
    /// By default filename is parl-daemon.log.
    /// File will be stored in logs directory, this is only the filename, not the full path.
    /// Log file is rotated every day and has the format YYYY-MM-DD.parl-daemon.log.
    #[clap(long, default_value_t = String::from("parl-daemon.log"))]
    filename_log: String,
    /// Logs directory
    /// 
...
async fn main() -> Result<()> {
    ...
    info!("PARL Blockchain running version: {}", VERSION);
    info!("----------------------------------------------");
...
fn build_prompt_message(prompt: &ShareablePrompt, topoheight: u64, median_topoheight: u64, network_hashrate: f64, peers_count: usize, rpc_count: usize, miners_count: usize, mempool: usize, network: Network) -> String {
    ...
    format!(
        "{} | {} | {} | {} | {} | {} | {} {}{} ",
        prompt.colorize_str(Color::Blue, "PARL"),
        topoheight_str,
    ...
    for topo in pruned_topoheight..=topoheight {
        ...
        let block_reward = if pruned_topoheight == 0 || topo - pruned_topoheight > STABLE_LIMIT {
            ...
            if block_reward != expected_block_reward {
                manager.error(format!("Block reward saved is incorrect for {} at topoheight {}, got {} while expecting {}", hash_at_topo, topo, format_parl(block_reward), format_parl(expected_block_reward)));
                return Ok(())
            }
            block_reward
        } else {
...
async fn show_balance<S: Storage>(manager: &CommandManager, mut arguments: ArgumentManager) -> Result<(), CommandError> {
    ...
    let asset = prompt.read_hash(
        prompt.colorize_str(Color::Green, "Asset (default PARL): ")
    ).await.ok();

    let asset = asset.unwrap_or(PARL_ASSET);
...
async fn status<S: Storage>(manager: &CommandManager, _: ArgumentManager) -> Result<(), CommandError> {
    ...
    manager.message(format!("Current Supply: {} PARL", format_parl(supply)));
    manager.message(format!("Current Block Reward: {} PARL", format_parl(get_block_reward(supply))));
